...
class Window {
 public:
  ...
  inline uint8_t getPerCent() const noexcept {
    return uint8_t((constrain(balance_, 0, fullTime_) / float(fullTime_)) *
                   100);
  }
  bool begin();
 ...
 private:
  ...
  struct OpeningRoutine : Routine {
    Window* w;
    void operator()() override { w->device_.setState(OPENING); };
  } openingRoutine_;
  friend class OpeningRoutine;

  struct ClosingRoutine : Routine {
    Window* w;
    void operator()() override { w->device_.setState(CLOSING); };
  } closingRoutine_;
  friend class ClosingRoutine;

  struct StoppingRoutine : Routine {
    Window* w;
    void operator()() override { w->device_.setState(STOPPED); };
  } stoppingRoutine_;
