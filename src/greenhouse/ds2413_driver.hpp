...
class Task {
 ...
 public:
  Task() = default;
  explicit Task(routine_t* beforeRoutine, uint32_t delay = 0,
                routine_t* afterRoutine = nullptr) noexcept;
  ...
  void loop();
  inline void setName(const __FlashStringHelper* name) noexcept {
    name_ = name;
  }
